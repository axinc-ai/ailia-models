...
def get_train_outputs(net, create_net, params):
    ...
    if (not args.aug):
        logger.info('extract train set features with no augmentation')
        aug_num = 1
    else:
    ...
    for i_aug in range(aug_num):
        for i_img in range(0, len(train_imgs), batch_size):
            # prepare input data
            imgs = []
            logger.info('from (%s ~ %s) on augmenation lap %d' % 
                        (train_imgs[i_img], 
            ...
            for image_path in train_imgs[i_img:i_img + batch_size]:
                ...
                if (not args.aug):
                    ...
                    img, _, _, _, _ = preprocess_aug(img)
                imgs.append(img)
        ...
        for i in range(0, len(args.input), batch_size):
            ...
            for image_path in args.input[i:i + batch_size]:
                ...
                if (not args.aug):
                    ...
                    img, img_resized, angle, pad_h, pad_w = preprocess_aug(img)
                ...
                if gt_type_dir:
                    ...
                    if os.path.exists(gt_fpath):
                        ...
                        if (not args.aug):
                            ...
                            gt_img, _, _, _, _ = preprocess_aug(gt_img, mask=True,
                                                                angle=angle,
