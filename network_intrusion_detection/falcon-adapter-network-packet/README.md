# falcon-adapter-network-packet

## Input

- Network Packet - HEX DATA
```

```

## Output

- Generated Tags
```
--- tags ---
considered regular expected (0.9684)
malicious intent (0.9616)
typical (0.9613)
reference point (0.9590)
label signifies normal network behavior (0.9576)
standard network protocols (0.9566)
baseline (0.9543)
Average (0.9535)
abnormal traffic patterns enabling (0.9528)
expected traffic patterns (0.9523)
------------
```

## Requirements

This model requires additional module.

```
pip3 install transformers
pip3 install nltk
pip3 install scapy
```

## Usage
Automatically downloads the onnx and prototxt files on the first run.
It is necessary to be connected to the Internet while downloading.

For the sample hex-data,
```bash
$ python3 falcon-adapter-network-packet.py
```

If you want to specify the input hex file, put the file path after the `--input` option.  
```bash
$ python3 falcon-adapter-network-packet.py --input HEX_FILE_PATH
```

If you want to specify the input hex data, put the hex value after the `--hex` option.  
```bash
$ python3 falcon-adapter-network-packet.py --hex 3ca6f6...069608
```

If you specify the `--save_vector` option, the calculated vector will be saved to npy files.  
From the second time onwards, this file will be loaded, so feature vector calculation can be omitted.
```bash
$ python3 falcon-adapter-network-packet.py --save_vector
```

## Reference

- [NIDS Transformers](https://github.com/rdpahalavan/nids-transformers)
- [Hugging Face - rdpahalavan/falcon-adapter-network-packet](https://huggingface.co/rdpahalavan/falcon-adapter-network-packet)

## Framework

Pytorch

## Model Format

ONNX opset=11

## Netron

[similar.onnx.prototxt](https://netron.app/?url=https://storage.googleapis.com/ailia-models/falcon-adapter-network-packet/similar.onnx.prototxt)
