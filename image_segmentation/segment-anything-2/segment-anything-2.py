...
def predict(
    ...
    mask_input, unnorm_coords, labels, unnorm_box = _prep_prompts(
       features, point_coords, point_labels, box, mask_input, normalize_coords
    )
...
def _prep_prompts(
    point_coords, point_labels, box, mask_logits, normalize_coords, img_idx=-1
):
...
def _predict(
    ...
    low_res_masks, iou_predictions, _, _ = model.run([_features["image_embed"][img_idx].unsqueeze(0), self.model.sam_prompt_encoder.get_dense_pe(), sparse_embeddings, dense_embeddings, multimask_output, batched_mode, high_res_features])
